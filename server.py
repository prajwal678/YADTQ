...
class YADTQServer:
    ...
    def verify_worker(self, worker_id):
        try:
            verification_msg = {"type": "ping", "timestamp": str(dt.now())}
            self.producer.send(
            ...
            while (time.time() - start_time) < self.verify_timeout:
                for msg in self.consumer_for_heartbeat:
                    if (msg.value.get("worker_id") == worker_id) and (
                        msg.value.get("type") == "ping"
                    ):
                ...
                with self.lock:
                    ...
                    for worker_id, worker_state in self.worker_states.items():
                        time_since_last_seen = (
                            now_time - worker_state["last_seen"]
                        ).total_seconds()
                    ...
                    try:
                        ...
                        try:
                            ...
                            if "." in start_time_str:
                                start_time = dt.fromisoformat(start_time_str.split("+")[0] if "+" in start_time_str else start_time_str)
                            else:
    ...
    def retry_task(self, task_id):
        try:
            ...
            # get the task args from the task data or from a backup store
            task_args = task_data.get("args", {})
            if not task_args:
                print(f"\ntask {task_id} args not found, using empty args")
                task_args = {}
            ...
            while True:
                ...
                with self.lock:
                    ...
                    for w_id, w_data in self.active_workers.items():
                        worker_status = (
                            "active" if w_data["status"] == "active" else "inactive"
                        )
                        task_status = (
                            f"doing {w_data['current_task']}"
                            if w_data["current_task"]
                            else "free"
                        )
                        time_since_last_seen = (
                            now_time - w_data["last_seen"]
                        ).total_seconds()
                        print(
